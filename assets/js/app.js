...
const serverProducts = Array.isArray(window.__POS_PRODUCTS__) ? window.__POS_PRODUCTS__ : [];
const activeVariants = Array.isArray(window.__POS_VARIANTS__) ? window.__POS_VARIANTS__ : [];
const receiptTemplate = (window.__RECEIPT_TEMPLATE__ && typeof window.__RECEIPT_TEMPLATE__ === 'object')
    ? window.__RECEIPT_TEMPLATE__
    : {};
const printTemplates = (window.__PRINT_TEMPLATES__ && typeof window.__PRINT_TEMPLATES__ === 'object')
...
const variantValidationNotice = document.getElementById('variantValidationNotice');
...
function renderCategoryButtons() {
    ...
    categoriesWrapper.innerHTML = allCats
        .map((cat) => `<button class="category-btn ${cat === activeCategory ? 'active' : ''}" data-cat="${cat}">${cat}</button>`)
        .join('');
...
function renderCashierVariantOptions() {
    cashierVariantSelect.innerHTML = '<option value="">Harga Normal (Utama)</option>';
    activeVariants.forEach((v) => {
...
function updateSummary() {
    ...
    if (hasUnsetVariant) {
        ...
        variantValidationNotice.textContent = 'Masih ada item dengan varian harga yang belum disetting. Set harga di master produk atau pilih varian lain.';
    } else {
...
function renderTransactionList() {
    ...
    const filter = transactionStatusFilter ? transactionStatusFilter.value : 'all';
    const rows = todayTransactions.filter((tx) => (filter === 'all' ? true : tx.status === filter));
    if (!rows.length) {
        transactionList.innerHTML = '<div style="font-size:12px;color:var(--text-muted);text-align:center;padding:8px 0;">Belum ada transaksi.</div>';
        return;
    }

    transactionList.innerHTML = rows
        .map((tx) => {
            const name = tx.customer_name && tx.customer_name.trim() !== '' ? tx.customer_name : 'Pelanggan Umum';
            const badge = tx.status === 'paid'
                ? '<span class="tx-badge paid">Sudah Bayar</span>'
                : '<span class="tx-badge pending">Belum Bayar</span>';
            const actionBtn = tx.status === 'pending'
...
window.loadDraftTransaction = async (id) => {
    try {
        ...
        cart = items.map((it) => ({
            ...
            key: cartKey(Number(it.product_id || 0), it.variant_id ? String(it.variant_id) : ''),
        }));
...
function loadCartFromStorage() {
    try {
        ...
        return saved.map((item) => normalizeSavedCartItem(item)).filter((item) => item && item.quantity > 0);
    } catch (_e) {
        return [];
    }
}
...
window.changeCartItemVariant = (index, variantId) => {
    ...
    const duplicateIndex = cart.findIndex((x, i) => i !== index && x.key === newKey);
...
function generateReceiptHTML(total, paid, change) {
    ...
    const itemsHTML = cart
        .map((item) => {
            const lineName = item.variant_name ? `${item.name} (${item.variant_name})` : item.name;
            const lineCode = showItemCode ? `<div style="font-size:10px;color:#666;">${item.code || ''}</div>` : '';
            return `
    ...
    if (printTemplates.nota && String(printTemplates.nota).trim() !== '') {
        const statusText = paid >= total && total > 0 ? 'Sudah Bayar' : 'Belum Bayar';
        const logoImg = logoUrl ? `<img src="${escapeHtml(logoUrl)}" alt="Logo" style="max-width:120px;max-height:60px;object-fit:contain;">` : '';
        return applyPrintTemplate(printTemplates.nota, {
            ...
            customer_name: escapeHtml(customerNameInput ? customerNameInput.value.trim() || 'Pelanggan Umum' : 'Pelanggan Umum'),
...
checkoutBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            renderCart();
            showToast(`Transaksi tersimpan (${result.invoice_no || 'OK'})`, 'success');
        }, 500);
    } catch (err) {
        showToast(`Gagal checkout: ${err.message}`, 'error');
    } finally {
        checkoutBtn.innerHTML = '<div class="checkout-btn-content"><span>Bayar Sekarang</span><span class="shortcut-hint-dark">Enter ↵</span></div>';
        updateSummary();
    }
});
...
const draftIdFromUrl = Number(new URLSearchParams(window.location.search).get('draft_id') || 0);
if (draftIdFromUrl > 0) {
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter') {
        if (document.activeElement === searchInput) {
            if (productsGrid.children.length > 0 && productsGrid.children[0].className === 'product-card') {
